...
!------------------------------------------------
    subroutine matrix_inverse(A, Ainv)
        ...
        !数値計算ではａｌｌｏｃａｔｅしたらｄｅａｌｌｏｃａｔｅするのが習慣らしい
...
!------------------------------------------------
    ...
    subroutine LUdecomposition(P, A, L, U)
        ...
        !ｎは行列のサイズ，ｉはＰとＬの初期値として１を導入する，ｋはピボット操作の回数
        integer :: n, i, k

        n = size(A, 1)

        !Ａは固定し，Ｕを動かして最終的にＰＡ＝ＬＵを目指す
        U = A

        !ＰとＬは最初に全成分を０にしてからｉを用いて対角成分に１を挿入
        P = 0.0d0
        L = 0.0d0
        do i = 1, n
            P(i, i) = 1.0d0
...
!-----------------------------------------------
    ...
    subroutine Pivot(P, L, U, k)
        ...
        !ｎは行列Ｕのサイズ，ｉはピボットとなる最大絶対値を探索するループで使う，ｐｉｖｏｔ＿ｒｏｗは最終的にｋ行と入れ替える行
        integer :: n, i, pivot_row

        !ｍａｘは探索する中での最大値，〇ｚは入れ替えを行う際基準となるｋ行を１行分丸ごと保存
        ...
        !Ｕのピボットを見つけたら合わせてＰ，Ｌ，Ｕの成分を入れ替える
        !ただし，Ｌに関しては確定した対角成分以外を入れ替えるため発動する条件を付与する
...
!-----------------------------------------------
    subroutine GaussElimination(L, U, k)
        ...
        !ｎは行列のサイズ，行列Ｕの成分をＵ（ｉ，ｊ）とし，それぞれの成分で計算を行う
        integer :: n, m, i, j
        !ｆａｃｔｏｒはピボット選択後ガウス消去を行うためにk行以外の行に掛ける係数
        real(real64) :: factor
...
!-----------------------------------------------
    subroutine Linverse(L, Linv)
        ...
        !ｎは行列Ｌのサイズ，Ｌの成分をＬ（ｉ，ｊ）とする
        integer :: n, m, i, j, k
        !ｓ（ｓｕｍ）は一時的に和を保存して，最終的にーＬ（ｊ，ｊ）で割る　数値計算でよく用いる手法
        real(real64) :: s

        n = size(L, 1)
        m = size(L, 2)

        !Ｌｉｎｖの中身は後から挿入するとして，一旦全成分を０で統一
        Linv = 0.0d0
